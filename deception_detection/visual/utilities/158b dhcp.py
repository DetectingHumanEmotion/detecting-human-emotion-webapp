...
from python_dhcp_server.server.listener import *


def get_host_ip_addresses():
    return gethostbyname_ex(gethostname())[2]
    # gethostbyname_ex(hostname) -- Only for IPV4
    #   returns (hostname, aliaslist, ipaddrlist)


# bootstrap protocol default values
class WriteBootProtocolPacket(object):
    ...
    vendor_class_identifier = 'PXEClient'  # set the vendor_class to PXEClient
    boot_file_name = 'foo'  # FIXME?
...
# Constructor to tie together the configuration file --  the dhcp options
    def __init__(self, configuration):
        self.tftp_server_name = configuration.tftp_user_addr
        for i in range(256):
    ...
    #Pack the data into bytes for packets
    def to_bytes(self):
...
class Transaction(object):
    ...
    # DHCP client discovered -- send offer
...
class DHCPServerConfiguration(object):
    ...
    tftp_user_addr = None  # placeholder for PXE config, used by piman
...
class DHCPServer(object):
    ...
    def __init__(self, configuration=None):
        ...
        self.socket.bind(('', 67))  # '' configuration.ip doesn't work - will get restricted ip access
...
# Set all the configurations and options and run the DHCP server
@click.command()
